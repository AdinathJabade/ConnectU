...
const StudentFields = ({ form, handleChange, showPassword, setShowPassword, errors = {} }) => {
    ...
    <>
      ...
      <div className="relative group mb-4" ref={el => (fieldsRef.current[1] = el)}>
        ...
        <input
          ...
          className={`w-full pl-10 px-4 py-3 border-2 rounded-lg focus:outline-none focus:border-indigo-500 transition bg-indigo-50 group-hover:shadow-lg ${errors.prn ? 'border-red-400' : 'border-indigo-200'}`}
          value={form.prn}
          onChange={handleChange}
          required
        />
        {errors.prn && <p className="text-red-500 text-xs mt-1 ml-2">{errors.prn}</p>}
      </div>
